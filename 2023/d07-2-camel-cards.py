...
def check_hand_strength(hand):
    ...
    hand_info = sorted([(card, hand.count(card)) for card in card_set if card !="J"], key= lambda card:card[1], reverse=True)
    if len(hand_info) == 0:
        return 0

    if "J" in hand:
        new_hand = sub("J", hand_info[0][0],hand)
        card_set = list(set(new_hand))
        hand_info = sorted([(card, new_hand.count(card)) for card in card_set],key=lambda card: card[1],reverse=True)
...
def tests():
    ...
    assert check_hand_strength("JJJJJ") == 0
